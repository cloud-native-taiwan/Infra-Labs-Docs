...
內部路由由 Arista SVI 處理運算/API VLAN，管理 NAT 則由 NFX250 VM 負責。
...
| 範圍 | 閘道 | 網路 |
|------|------|------|
| 管理 VLAN 1 | NFX250 VM (192.168.0.1) | 192.168.0.0/24 |
...
NFX250 VM 為需要存取網際網路的管理平面流量（套件更新、NTP 等）提供 NAT：

| 參數 | 值 |
|------|-----|
| NAT 閘道 | NFX250 VM |
| 公用介面 | ens3 -- 103.122.117.253 |
