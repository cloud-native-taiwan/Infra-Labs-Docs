...
4. Pushgateway: PushGateway: 由於 PushGateway 在監控多個實體時，可能成為單點故障或潛在瓶頸，且某種程度上也失去 Prometheus 自動監控各實體的設計，因此需相當注意使用時機；通常只用於不適用 pull 機制的場景，例如 service-level 的 batch job。
